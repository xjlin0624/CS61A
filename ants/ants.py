...
class Ant(Insect):
    ...
    def __init__(self, health=1, buffed=False):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curr, i = self.place, 0
        while curr is not beehive and i <= self.max_range: 
            if len(curr.bees) > 0 and i >= self.min_range: 
                return bee_selector(curr.bees)
            curr, i = curr.entrance, i + 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - (len(self.place.bees) * amount) <= 0:
            for i in bee_lst:
...
class HungryAnt(Ant):
    ...
    After eating a Bee, it must spend 3 turns chewing before eating again. 
    ...
    def action(self, gamestate):
        if self.chewing != 0: 
            ...
            if len(self.place.bees) > 0:
                bee_selector(self.place.bees)\
                    .reduce_health(bee_selector(self.place.bees).health)
...
class TankAnt(ContainerAnt):
    """TankAnt is a container that protects an ant in its place and also 
...
# END Problem EC
    ...
    def remove_from(self, place):
        ...
        if not self.impostor: 
            """Does nothing. Real queen cannot be removed."""
        else:
            # If imposter, default to normal behavior
            Ant.remove_from(self, place)
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.impostor:
            ...
            while curr_place.exit:
                ...
                if curr_ant:
                    curr_ant.buff()
                    if isinstance(curr_ant, ContainerAnt) and curr_ant.contained_ant:
                        curr_ant.contained_ant.buff()                        
        # END Problem EC

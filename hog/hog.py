...
def more_boar(player_score, opponent_score):
    ...
    p_score = str(player_score)
    opp_score = str(opponent_score)
    if len(p_score) == 1:
        p_score = '0' + str(player_score)
        if len(opp_score) == 1:
            opp_score = '0' + str(opponent_score)
    elif len(opp_score) == 1:
        opp_score = '0' + str(opponent_score)
    return int(p_score[0]) < int(opp_score[0]) and int(p_score[1]) < int(opp_score[1])
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say_highest(score0, score1):
        ...
        if (current_score - last) > h:
            h = current_score - last
            print("Player", who, "has reached a new maximum point gain.", (current_score - last), "point(s)!")
        last = current_score
        return announce_highest(who, last_score=last, running_high=h)
    return say_highest
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    piggy_pts = piggy_points(opponent_score)
    if more_boar((score + piggy_pts), opponent_score) or piggy_pts >= cutoff:
        return 0
    else:
        return num_rolls  # Replace this statement

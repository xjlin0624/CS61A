...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(op, new_lst, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    2
    """
    # BEGIN PROBLEM 7  # replace this with lines of your own code
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        func_body = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(func_name, func_body)
        return func_name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil: 
        ...
        if is_true_primitive(first_eval):
            result = first_eval
            expressions = expressions.rest
        else: 
            return False
